...
(function () {
  ...
  const gmGet = (k, d) =>
    typeof GM_getValue !== 'undefined'
      ? GM_getValue(k, d)
      : (localStorage.getItem('ytm_beautifier_' + k) ?? JSON.stringify(d)) &&
        JSON.parse(
          localStorage.getItem('ytm_beautifier_' + k) || JSON.stringify(d)
        );
  const gmSet = (k, v) =>
  ...
  function fetchLyrics(title, artist, album, year, reroll = false) {
    ...
    gmXhr({
      ...
      onload: (r) => parseLyricsResponse(r?.responseText || ''),
      onerror: () => {
  ...
  function parseLyricsResponse(text) {
    if (!text || text === 'no_lyrics_found' || text.includes('<title>500')) {
    ...
    try {
      ...
      const data = res.lrc;
      if (res.source === 'unofficial' && typeof data === 'string')
  ...
  function parseLRC(txtLrc) {
    ...
    lines.forEach((l) => {
      const m = l.match(/^\[(\d+):(\d+)\](.*)$/);
      if (m) {
        times.push(parseInt(m[1], 10) * 60 + parseInt(m[2], 10));
        lyrics.push(m[3].trim() || '♪♪');
      }
    });
    sanitizeAndBuild();
  }
...
@media(max-width:768px){#ytm-lyrics-card{width:300px;right:10px;top:10px;max-height:70vh}#ytm-launcher{top:10px;right:10px;padding:8px 12px}}`);
  ...
  function buildLyricsCard() {
    ...
    ctr.appendChild(minB);
    ctr.appendChild(closeB);
  ...
  function tryMediaSeek(target) {
    try {
      const media = Array.from(document.querySelectorAll('video,audio')).filter(
        (m) => m && m.duration > 0 && m.offsetParent !== null
      )[0];
    ...
    for (const sel of selectors) {
      ...
      try {
        if (el.value !== undefined) {
          const v = pct * 100;
          el.value = v;
          ['input', 'change', 'slide', 'iron-change'].forEach((t) =>
            el.dispatchEvent(
              new CustomEvent(t, { bubbles: true, detail: { value: v } })
            )
          );
          if (el.value == v) return true;
        }
        if (el._setValue) {
          el._setValue(pct * 100);
          return true;
        }
        if (el.immediateValue !== undefined) {
          el.immediateValue = pct * 100;
          el.value = pct * 100;
          return true;
        }
        // pointer events
